        ...
        @foreach($modelSearchResults as $item)
            ...
            <div class="card-iniciativa position-relative bg-lighter rounded-4 p-16 pt-md-40 p-md-32 mb-24">
                <div class="text-right">
                    <p>
                        {{ Date::parse($item->searchable->fecha_presentacion)->format('d') }} de
                        {{ ucfirst(Date::parse($item->searchable->fecha_presentacion)->formatLocalized('%B'))}}
                        ,
                        {{ Date::parse($item->searchable->fecha_presentacion)->format('Y') }}
                    </p>
                </div>
                <div class="list-diputados d-flex flex-wrap">
                    @if(count($item->searchable->autor))
                        @foreach($item->searchable->autor as $autor)
                            <div class="d-flex align-items-center bg-light py-12 px-16 rounded-4 me-16 mb-16">
                                <figure class="m-0 me-lg-12">
                                    <img
                                        src="{{asset($autor->foto_autor)}}" alt=""
                                        class="rounded-circle border border-primary">
                                </figure>
                                <p class="d-none d-lg-block mb-0 fw-semibold">{{$autor->name}}</p>
                            </div>
                        @endforeach
                    @endif
                </div>
                <div class="bg-light p-16 p-lg-32 rounded-4">
                    <div class="fs-lg-18"><p>{{$item->searchable->iniciativa}}</p></div>
                    <div class="scrollable-x-panel">
                        <div class="d-flex flex-wrap fs-13 fw-semibold">
                            @foreach($item->searchable->comision as $comision)
