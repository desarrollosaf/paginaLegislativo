            ...
            <div class="modal-body">
                <form id="form-autores">
                    ...
                    <div class="niveles comision">
                        <div class="col">
                            {!! Form::label('autor', 'Anfitrion:', array('class' => 'form-label')) !!}
                            {!! Form::select('autor', $selects[1], null,['class'=>'js-example-basic-multiple form-control select-autor', "multiple" => "multiple", "id" => "comision", "data-width"=> "100%"]) !!}
                        </div>
                    </div>
                    <div class="niveles municipio">
                        <div class="col">
                            {!! Form::label('autor', 'Anfitrion:', array('class' => 'form-label')) !!}
                            {!! Form::select('autor', $selects[2], null,['class'=>'js-example-basic-multiple form-control select-autor', "multiple" => "multiple", "id" => "municipio", "data-width"=> "100%"]) !!}
                        </div>
                    </div>
                    <div class="niveles partido">
                        <div class="col">
                            {!! Form::label('autor', 'Anfitrion:', array('class' => 'form-label')) !!}
                            {!! Form::select('autor', $selects[3], null,['class'=>'js-example-basic-multiple form-control select-autor', "multiple" => "multiple", "id" => "partido", "data-width"=> "100%"]) !!}
                        </div>
                    </div>
                    <div class="niveles diputado">
                        <div class="col">
                            {!! Form::label('autor', 'Anfitrion:', array('class' => 'form-label')) !!}
                            {!! Form::select('autor', $selects[4], null,['class'=>'js-example-basic-multiple form-control select-autor', "multiple" => "multiple", "id" => "diputado", "data-width"=> "100%"]) !!}
                        </div>
                    </div>
                    <div class="niveles legislatura">
                        <div class="col">
                            {!! Form::label('autor', 'Autor:', array('class' => 'form-label')) !!}
                            {!! Form::select('autor', $selects[7], null,['class'=>'js-example-basic-multiple form-select select-autor', "id" => "legislatura", "data-width"=> "100%" ]) !!}
                        </div>
                    </div>
                    <div class="niveles otros">
                        <div class="col">
                            {!! Form::label('autor', 'Autor:', array('class' => 'form-label')) !!}
                            {!! Form::select('autor', $selects[6], null,['class'=>'js-example-basic-multiple form-select select-autor', "id" => "otros", "multiple" => "multiple", "data-width"=> "100%" ]) !!}
                        </div>
                    </div>
                </form>
            </div>
...
@push("page-scripts")
    <script type="text/javascript">
        var autores = [];
        @if(isset($agenda) && count($agenda->anfitrion))
        @foreach($agenda->anfitrion as $key => $item)
        autores.push(
            {
        ...
        $(document).ready(function () {
            ...
            $("#form-autores").validate({
                ...
                submitHandler: function () {
                    ...
                    let autor = $(".tipo-autor-select-active option:selected").text();
                    let nivel = $("#select-nivel option:selected").text();
                    let nivel_autor_id = $("#select-nivel option:selected").val();
                    let idAutor = $(".tipo-autor-select-active option:selected").val();
                    var exist = autores.filter(name => name.autor.includes(autor))
